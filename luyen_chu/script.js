...
// code nut ctrl z trên đt - laptop tự ẩn 
// });

// skipBtn.addEventListener('click', () => {
// // Tính năng Bỏ qua (Skip)
// if (skipBtn) {
//     skipBtn.addEventListener('click', () => {
//         // Kiểm tra xem danh sách đang luyện có tồn tại và còn chữ không
//         if (activeList && activeList.length > 0) {
            
//             // 1. Rút phần tử hiện tại ra khỏi danh sách
//             const skippedKana = activeList.splice(currentIdx, 1)[0]; 
            
//             // 2. Đẩy phần tử vừa rút xuống cuối danh sách
//             activeList.push(skippedKana);
            
//             // Lưu ý: Ta KHÔNG tăng currentIdx lên. 
//             // Vì mảng đã bị rút phần tử tại currentIdx, nên phần tử kế tiếp đã tự động bị đẩy lên nằm ở vị trí currentIdx rồi.
            
//             // 3. Hiển thị chữ cái mới (chữ kế tiếp vừa bị đẩy lên)
//             showCurrentKana();
            
//             // 4. Xóa canvas để chuẩn bị viết chữ mới
//             clearCanvasAndState();
//         }
//     });
// }
...
window.addEventListener('keydown', function(event) {
    // Xoá: esc, delete, x
    if (event.key === "Delete" || event.keyCode === 46 || event.key === "Escape" || event.keyCode === 27 || event.key.toLowerCase() === "x") {
        clearCanvasAndState();
    }
    // Tiếp: n, v
    if (event.key.toLowerCase() === "n" || event.key.toLowerCase() === "v") {
        if (typeof nextBtn !== "undefined" && infoBox.style.display !== 'none') {
            nextBtn.click();
        }
    }
...
window.onload = (event) => {
  ...
  // Kiểm tra trạng thái ẩn/hiện của nút ctrl_z khi load trang
  if (ctrl_z) {
      ctrl_z.style.display = (window.innerWidth <= 900) ? 'inline-block' : 'none';
  }
};
...
window.addEventListener('resize', () => {
    ...
    // ĐIỀU KIỆN ẨN/HIỆN NÚT UNDO KHI XOAY MÀN HÌNH HOẶC RESIZE TRÌNH DUYỆT
    if (ctrl_z) {
        ctrl_z.style.display = (window.innerWidth <= 900) ? 'inline-block' : 'none';
    }
});
